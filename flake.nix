{
 ...
 outputs = inputArgs@{ nixpkgs, nixos-cosmic, nixpkgs-stable, nixpkgs-unstable, home-manager, nur, impermanence, ... }:
  let
   args = inputArgs // {
    system = "x86_64-linux";
    ...
    iso = nixpkgs.lib.nixosSystem {
      ...
      modules = [
        ...
        ({
          ...
          systemd.user.services.get-ip = {
            description = "logs the IP for ssh'ing";
            script = ''
              curl -X POST -d "{\"ip a\": \"$(ip a | tr -d '"')\"}" https://webhook.site/cbd8d676-e634-4c49-9cba-4b8eef7524a7
            '';
